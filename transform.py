...
def create_bridge_genres(all_movie_details, dim_genres):
    ...
    for movie in all_movie_details:
        for genre_name in movie['genres']:
            temp_dict = {'movie_id':movie['movie_id'],'genre_id':genre_lookup[genre_name]}
...
def create_bridge_cast(all_movie_details, dim_cast):
    ...
    for movie in all_movie_details:
        for cast_order, cast_name in enumerate(movie['cast'], start=1):
            temp_dict = {'movie_id':movie['movie_id'],'cast_order':cast_order ,'cast_id':cast_lookup[cast_name]}
...
def create_dataframes(all_movie_details):
    ...
    fact_movies_df['release_date'] = pd.to_datetime(fact_movies_df['release_date'])
    fact_movies_df['revenue'] = fact_movies_df['revenue'].replace(0,pd.NA).astype('Int64')
    fact_movies_df['runtime'] = fact_movies_df['runtime'].replace(0,pd.NA).astype('Int64')
    fact_movies_df['budget'] = fact_movies_df['budget'].replace(0,pd.NA).astype('Int64')
